...
if ( ! class_exists( 'Penci_WooCommerce' ) ) :
	class Penci_WooCommerce {
		...
		/**
		 * Change number of columns in shop page.
		 * @return int
		 */
		public function loop_shop_columns() {

			if ( 'no-sidebar' == penci_get_setting( 'penci_woo_sidebar_shop' ) || ! is_active_sidebar( 'penci-shop-sidebar' ) ) {
				return 4;
			}
		...
		public function add_body_classes( $classes ) {
			...
			$number_column = $this->loop_shop_columns();
		 ...
		 *
		 */
		function custom_products_per_page( $options ) {
			$options = 24;
			if ( penci_get_setting( 'penci_woo_post_per_page' ) ) {
				$options = absint( penci_get_setting( 'penci_woo_post_per_page' ) );
			}
		...
		public function shop_posts( $query ) {
			if ( ! is_admin() && $query->is_main_query() && $query->is_post_type_archive( 'product' ) ) {
				$posts_per_page = penci_get_setting( 'penci_woo_post_per_page' );
