...
function penci_get_categories() {
	...
	if( penci_get_theme_mod( 'archive_show_pri_cat' )&& class_exists( 'WPSEO_Primary_Term' ) ) {
		...
		echo ( '<span class="penci-cat-links"><a class="penci-cat-name" href="' . $category_link . '" title="' . sprintf( esc_html__( "View all posts in %s", 'pennews' ), htmlspecialchars( $category_display ) ) . '" ' . '>' . $category_display . '</a></span>' );
