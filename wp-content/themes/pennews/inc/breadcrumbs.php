...
function penci_breadcrumbs( $args = '' )
{
	...
	if ( is_home() && ! is_front_page() )
	{
		...
		if ( $args['display_last_item'] )
			$items[] = sprintf( $item_text_tpl, $current_term_label );
	...
	elseif ( is_search() )
	{
		$items[] = sprintf( $item_text_tpl, sprintf( esc_html__( '%s &quot;%s&quot;', 'pennews' ), penci_get_tran_setting( 'penci_search_title' ), get_search_query() ) );
	}
	elseif ( is_404() )
	{
		$items[] = sprintf( $item_text_tpl, penci_get_tran_setting( 'penci_breadcrumb_page_404' ) );
	}
	elseif ( is_author() )
	{
